...
/*
  Function: pillar pooling (max) - SYCL kernel
  Args:
    b                : batch size
...
*/
void bev_max_pool_kernel(sycl::nd_item<1> item, int b, int d, int h, int w, int n, int c, int n_intervals,
...
/*
  Function: pillar pooling backward (max) - SYCL kernel
  Args:
    b                : batch size
    d                : depth of the feature map
    h                : height of pooled feature map
...
*/
void bev_max_pool_grad_kernel(sycl::nd_item<1> item, int b, int d, int h, int w, int n, int c, int n_intervals,
...
void bev_max_pool(int b, int d, int h, int w, int n, int c, int n_intervals, const float* geom_feats,
  ...
  q.parallel_for(sycl::nd_range<1>(global_size, local_size), [=](sycl::nd_item<1> item) {
    bev_max_pool_kernel(item, b, d, h, w, n, c, n_intervals, geom_feats, geom_coords,
...
void bev_max_pool_grad(int b, int d, int h, int w, int n, int c, int n_intervals, const float* out_grad,
  ...
  q.parallel_for(sycl::nd_range<1>(global_size, local_size), [=](sycl::nd_item<1> item) {
    bev_max_pool_grad_kernel(item, b, d, h, w, n, c, n_intervals, out_grad, geom_coords,
